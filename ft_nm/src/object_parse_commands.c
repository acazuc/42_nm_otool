...
/*   Updated: 2016/09/20 13:59:04 by acazuc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_nm.h"

static int		object_parse_command_execute(t_object *object
		, struct load_command *load_command)
{
	if (load_command->cmd == LC_SYMTAB)
		return (object_parse_command_symtab(object));
...
static int		object_parse_command(t_object *object)
{
	struct load_command	load_command;
	size_t				buff_pos;
	...
	if (!object_parse_command_execute(object, &load_command))
		return (0);
...
int				object_parse_commands(t_object *object)
{
	...
	{
		if (!object_parse_command(object))
			return (0);
		i++;
	}
	return (1);
